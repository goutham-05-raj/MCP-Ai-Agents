...
def generate_ics_content(plan_text: str, start_date: datetime = None) -> bytes:
    ...
    """
    cal = Calendar()
    cal.add('prodid','-//TripCraft AI Planner//github.com//')
    cal.add('version', '2.0')
...
async def run_mcp_travel_planner(destination: str, num_days: int, preferences: str, budget: int, openai_key: str, google_maps_key: str):
    ...
    try:
        ...
        travel_planner = Agent(
            name="TripCraft Planner",
            role="Creates smart travel itineraries using Airbnb listings, Google Maps navigation, and live web search",
            model=OpenAIChat(id="gpt-4o", api_key=openai_key),
            description=dedent(
                """\
                You are TripCraft, an intelligent travel consultant AI that builds highly personalised itineraries without asking unnecessary questions.

                You have access to:
                🏨 Airbnb — real listings with live availability and current pricing
                🗺️ Google Maps MCP — directions, distances, and local navigation data
                🔍 Live web search — current reviews, events, and travel advisories

                ALWAYS produce a complete, detailed itinerary immediately. Make smart assumptions when data is incomplete.
                Use Google Maps MCP extensively to calculate distances and travel times between every location.
                """
            ),
            instructions=[
...
# Configure the page
st.set_page_config(
    page_title="TripCraft AI Planner",
    page_icon="🌍",
    layout="wide"
)
...
st.title("🌍 TripCraft AI Planner")
st.caption("Plan your perfect trip with TripCraft — powered by MCP servers for live Airbnb, Maps, and web data")
                ...
                with st.spinner(tools_message):
                    try:
                        ...
                        if "Airbnb" in response and ("listing" in response.lower() or "accommodation" in response.lower()):
                            st.success("✅ Your TripCraft itinerary is ready — with live Airbnb data!")
                            st.info("🏨 Real Airbnb listings were used for accommodation recommendations")
                        else:
                            st.success("✅ Your TripCraft itinerary is ready!")
                            st.info("📝 Accommodation suggestions based on curated knowledge (Airbnb MCP connection may have timed out)")
    ...
    with col2:
        if st.session_state.itinerary:
            ...
            st.download_button(
                ...
                data=ics_content,
                file_name="tripcraft_itinerary.ics",
                mime="text/calendar"
            )

    # Display itinerary
    if st.session_state.itinerary:
        st.header("📋 Your TripCraft Itinerary")
