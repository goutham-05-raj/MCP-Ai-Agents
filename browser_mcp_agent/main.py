...
# Page config
st.set_page_config(page_title="🌐 WebPilot — Browser MCP Agent", page_icon="🌐", layout="wide")

# Title and description
st.markdown("<h1 class='main-header'>🌐 WebPilot — Browser MCP Agent</h1>", unsafe_allow_html=True)
st.markdown("Control a real browser with plain English — WebPilot navigates, clicks, scrolls, and extracts web data for you")
...
with st.sidebar:
    ...
    st.markdown("- Go to news.ycombinator.com")
    st.markdown("- Open wikipedia.org and search for 'Model Context Protocol'")
    
    st.markdown("**Interactions**")
    st.markdown("- Click on the first article and summarise it")
    st.markdown("- Scroll down and list all visible headlines")
    
    st.markdown("**Multi-step Tasks**")
    st.markdown("- Go to github.com/trending, scroll down, and list top 5 repos")
    st.markdown("- Search Google for 'MCP agents 2025' and summarise top results")
    
    st.markdown("---")
    st.caption("WebPilot uses Playwright to control a real Chromium browser instance.")
    ...
    if not st.session_state.initialized:
        try:
            ...
            st.session_state.browser_agent = Agent(
                name="webpilot",
                instruction="""You are WebPilot, an intelligent web browsing assistant powered by Playwright.
                    - Navigate to any website and perform real browser actions: click, scroll, type, hover
                    - Extract and summarise information from web pages clearly
                    - Take screenshots of relevant page sections when helpful
                    - Execute multi-step browsing workflows efficiently
                    - Always confirm the outcome of each action in your response
                    
                Respond with a clear status update after completing each command.""",
...
# Footer
st.markdown("---")
st.write("🌐 WebPilot — Built with Streamlit, Playwright, and the [MCP-Agent](https://www.github.com/lastmile-ai/mcp-agent) Framework")
